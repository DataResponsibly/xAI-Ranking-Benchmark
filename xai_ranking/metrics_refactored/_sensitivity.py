...
from ._base import (
    _find_neighbors, _get_importance_mask, _MEASURES
)


def _pairwise_outcome_sensitivity(
    row_data1, row_data2, row_cont1, row_cont2, score_func, original_scores,
    threshold, n_tests, stds, rng
):
    ...
    rows_pert1 = np.array(
        [row_data1 + pert * mask1 for pert in perturbations]
    )
    rows_pert2 = np.array(
        [row_data2 + pert * mask2 for pert in perturbations]
    )
    ...
    for rows_pert in [rows_pert1, rows_pert2]:
        ...
        rows_pert_rank = np.array([
            scores_to_ordering(
                np.append(original_scores, score)
            )[-1]
...
def row_based_outcome_sensitivity(
    original_data, rankings, original_scores, score_func, contributions, row_idx,
    threshold=0.8, n_neighbors=10, n_tests=10, std_multiplier=0.2, random_state=None
):
    ...
    for i in range(len(data_neighbors)):
        ...
        score = _pairwise_outcome_sensitivity(
            original_data[row_idx], data_neighbors[i], row_cont1, row_cont2, score_func,
            original_scores, threshold, n_tests, stds, rng
        )
        scores.append(score)
...
def outcome_sensitivity(
    original_data, score_func, contributions, threshold=0.8, n_neighbors=10,
    n_tests=10, std_multiplier=0.2, aggregate_results=False, random_state=None
):
    ...
    sensitivities = np.vectorize(
        lambda row_idx: row_based_outcome_sensitivity(
            original_data, rankings, original_scores, score_func, contributions, row_idx,
            threshold, n_neighbors, n_tests, std_multiplier, random_state
        )
    )(
...
def row_wise_explanation_sensitivity(
    original_data, contributions, row_idx, rankings, n_neighbors=10,
    agg_type="mean", measure="kendall", **kwargs
):
    row_cont = np.array(contributions)[row_idx]
    ...
    distances = np.apply_along_axis(
        lambda row: _MEASURES[measure](row, row_cont, **kwargs), 1, cont_neighbors
    )
