...
def bootstrapped_explanation_consistency(
    population_result, batch_results, measure="kendall", axis=None, **kwargs
):
    batch_agreement = []
    ...
    sem = (
        np.std(batch_agreement) / np.sqrt(batch_agreement.size)
    )
    return mean, sem
...
def cross_method_explanation_consistency(
    ...
    mean = res_.mean()
    sem = (
        np.std(res_) / np.sqrt(res_.size)
    )
    return mean, sem


def _row_based_outcome_consistency(
    original_data, original_scores, score_func, contributions1, contributions2, row_idx,
    threshold, n_tests, stds, rng
):
    return _pairwise_outcome_sensitivity(
        original_data[row_idx], original_data[row_idx], contributions1[row_idx],
        contributions2[row_idx], score_func, original_scores, threshold,
        n_tests, stds, rng
    )


def cross_method_outcome_consistency(
    original_data, score_func, contributions1, contributions2, threshold=0.8,
    n_tests=10, std_multiplier=0.2, random_state=None
):
    ...
    consistencies = np.vectorize(
        lambda row_idx: _row_based_outcome_consistency(
            original_data, original_scores, score_func, contributions1,
            contributions2, row_idx, threshold, n_tests, stds, rng
        )
    )(
