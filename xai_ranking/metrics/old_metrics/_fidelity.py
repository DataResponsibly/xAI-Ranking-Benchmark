...
def row_wise_fidelity(
    ...
    rows_perturbed = np.array(
        [
            row_data + rng.normal(loc=0, scale=stds) * feature_mask
    ...
    rows_perturbed_rank = np.array(
        [
            scores_to_ordering(np.append(original_data_score, score))[-1]
...
def fidelity(
    ...
    if aggregate_results:
        return np.mean(sensitivities), np.std(sensitivities) / np.sqrt(
            sensitivities.size
        )
    else:
