...
def shapley_values(d, weights, upper_bound, lower_bound, features, num_batches, calculate_std=None):
    """
    input: df_all, weight
    """
    df = d.copy()
    ...
    for n, group in grouped:
        if num_batches == 1:
            ...
            if calculate_std:
                mean_contri = [group[attr].abs().std() for attr in contri_attributes]
                privileged_mean_contri = [
                    group.query('Group == "privileged"')[attr].abs().std()
                    for attr in contri_attributes
                ]
                protected_mean_contri = [
                    group.query('Group == "protected"')[attr].abs().std()
                    for attr in contri_attributes
                ]
            else:
...
def competing_powers(d, weights, upper_bound, lower_bound, features, num_batches, calculate_std=None):
    """
...
def competing_powers2(
    d, weights, upper_bound, lower_bound, features, num_batches, exponential=1, calculate_std=None
):
    """
    input: df_all, weights
